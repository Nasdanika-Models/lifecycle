...
Base classes come from the [IAM model](../../ai-dashboard/iam/iam.xcore) via the [seal model](../../ai-dashboard/seal/seal.xcore): `Staged extends SealedElement`, and the seal dependency is typed — signed sojourns are how approvals attach to transitions. The [decision analysis model](../decision-analysis/README.md) re-parents its `Comparand` to `Staged`, carrying the chain upward (nxcore < role < iam < seal < lifecycle < decision-analysis < decision-binding < ...). The Product Management model's `Lifecycle` is the ancestor this model reclaims and generalizes, the same reclaim-in-place pattern as the architecture model; the PM model is expected to eventually consume this floor rather than carry its own enumeration. Workflow schemes proper — mapping lifecycles to element kinds wholesale — belong to the floors that own kinds (architecture `ElementKind` catalogs), not here.
